...
const Neo4jQueryDetails: NextPage = () => {
  ...
  const [isEnabledQueryNodeResults, setIsEnabledQueryNodeResults] = React.useState(false);
  ...
  const queryResults = api.neo4j.getDatabaseResults.useQuery( {connectionString, usernameValue, passwordValue, nodeName}, {enabled: Boolean(isEnabledQueryNodeResults)});
  const querySpecificResults = api.neo4j.getDatabaseSpecificResults.useQuery( {connectionString, usernameValue, passwordValue, nodeName, nodeValue, filterValue}, {enabled: Boolean(isEnabledQuerySpecificResults)});
...
//TODO FIX THIS
 useEffect(() => {
  ...
  const resultRows: GridRowsProp = useMemo(
    () => rowValues.map((row, index) => ({ row, id: index }))  || [],
